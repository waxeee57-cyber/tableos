      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-1 flex-wrap">
            ...
            {item.is_new && <span className="text-xs px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded font-medium">ÚJ</span>}
            {item.is_popular && <span className="text-xs px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded font-medium">Népszerű</span>}
          </div>
          ...
          {item.prices.map((p) => (
            <div key={`${p.size}-${p.price}`} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {isOpen && (
                  <button
                    onClick={() => onAdd(item, p.price, p.size)}
